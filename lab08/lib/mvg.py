from numpy import average, cov, eye, float64, log, ones, stack, uint8, unique, zeros
from sklearn.metrics import confusion_matrix
from sklearn.preprocessing import normalize

from .normal import log_normal_density
from .types import F64Matrix, U8Array, U8Matrix
...
class Mvg:
    ...
    def inference(
        ...
        predicted = log_joint.argmax(axis=0)

        if ground_truth is None:
            print(type(predicted))
            return predicted
        return (predicted != ground_truth).sum() / data.shape[1]
    ...
    def confusion(
        self, data: F64Matrix, ground_truth: U8Array, prior="uniform"
    ) -> U8Matrix:
        predictions = self.inference(data, prior=prior)
        assert predictions.size == ground_truth.size
        return confusion_matrix(ground_truth, predictions).T
