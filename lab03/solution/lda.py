...
def load_iris():  # Same as in pca script
    return (
        cast(ndarray, load_iris()["data"].T),
        cast(ndarray, load_iris()["target"]),
    )
...
if __name__ == "__main__":
    ...
    print(U)
    print(
        compute_lda_JointDiag(D, L, m=2)
    )  # May have different signs for the different directions
    USol = numpy.load(
        "IRIS_LDA_matrix_m2.npy"
    )  # May have different signs for different directions
    print(USol)
